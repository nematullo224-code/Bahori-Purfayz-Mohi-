/* ==========================================================================
   ...
   WHATSAPP & PHONE: +992 90 857 96 96
   ========================================================================== */

const WHATSAPP_PHONE = '992908579696';
const DISPLAY_PHONE = '+992 90 857 96 96';
...
const TRANSLATIONS = {
  tg: {
    ...
    nav_contact: "Тамос",
    header_order_btn: "Фармоиш: +992 90 857 96 96",
    ...
    send_whatsapp_btn: "Фармоиш ба WhatsApp (+992 90 857 96 96)",
    ...
    faq_a4: "Шумо метавонед тавассути сомона тугмаи WhatsApp-ро пахш намуда (+992 90 857 96 96) мустақиман фармоиш диҳед ё ба рақамҳои мо занг занед.",
  ...
  ru: {
    ...
    header_order_btn: "Заказ: +992 90 857 96 96",
    ...
    send_whatsapp_btn: "Заказать в WhatsApp (+992 90 857 96 96)",
    ...
    faq_a4: "Нажмите кнопку WhatsApp (+992 90 857 96 96) на сайте или позвоните по указанным номерам телефона.",
  ...
  uz: {
    ...
    nav_contact: "Aloqa",
    header_order_btn: "Buyurtma: +992 90 857 96 96",
    ...
    send_whatsapp_btn: "WhatsApp ga yuborish (+992 90 857 96 96)",
    ...
    faq_a4: "Saytdagi WhatsApp tugmasini bosing (+992 90 857 96 96) yoki telefon raqamlarimizga qo'ng'iroq qiling.",
...
const PRODUCTS_DATA = [
  ...
  },
  {
    ...
    badge: "100% Зинда 🐟",
    mainImage: "fish_animated.png",
    gallery: [
      { src: "fish_animated.png", title: "Моҳии аслии соми ҶДММ Баҳори Пурфайз" },
...
class PureTransparentCatfishAquarium {
  constructor(canvasId) {
    ...
    // FPS throttle: 30fps on mobile, 60fps on desktop
    this.isMobile = window.innerWidth < 768;
    this.fpsInterval = this.isMobile ? 1000 / 30 : 1000 / 60;
    this.lastFpsCheck = 0;
    ...
    rawImg.src = 'fish_animated.png';
    rawImg.onerror = () => { rawImg.src = 'fish_animated.png'; };
  ...
  initFishes() {
    // Fewer fish on mobile to preserve battery/CPU
  ...
  animate(now) {
    ...
    this.lastFpsCheck = now - (elapsed % this.fpsInterval);
...
class RealCatfishEntity {
  ...
  draw(ctx, time, transparentSprite, isReady, isLight) {
    ...
    if (isReady && transparentSprite) {
      // Optimized: 10 slices (was 20) — same visual quality, 2x faster
      const slices = 10;
      const sliceW = drawW / slices;
      const spriteSliceW = transparentSprite.width / slices;
      for (let i = 0; i < slices; i++) {
        const xPos = -drawW * 0.5 + (i * sliceW);
        const normPos = i / slices;
        const wiggle = Math.sin(time * 10 - normPos * 5 + this.index) * (14 * (1 - normPos));
        ctx.drawImage(
          transparentSprite,
          spriteSliceW * i, 0,
          spriteSliceW, transparentSprite.height,
...
class BahoriPurfayzApp {
  ...
  initAnimations() {
    ...
    // Save observer for dynamically added elements (like product cards)
  ...
  renderProducts() {
    ...
    this.productsContainer.innerHTML = filtered.map((product, index) => {
      ...
      // Stagger animation delay for cards based on index
      ...
      return `
        <div class="product-card-3d reveal-element ${delayClass}" data-id="${product.id}">
    ...
    // Use event delegation on the container — one listener handles ALL cards
    this.productsContainer.addEventListener('click', (e) => {
      const viewBtn = e.target.closest('[data-view-id]');
      if (viewBtn) { this.openProductModal(viewBtn.getAttribute('data-view-id')); return; }
      const orderBtn = e.target.closest('[data-order-id]');
      if (orderBtn) { this.directWhatsAppOrder(orderBtn.getAttribute('data-order-id')); return; }
      const thumb = e.target.closest('.thumb-mini');
      if (thumb) {
        e.stopPropagation();
    ...
    if (this.scrollObserver) {
      this.productsContainer.querySelectorAll('.product-card-3d').forEach(card => {
  ...
  bindEvents() {
    ...
    if (this.productSearch) {
      ...
      this.productSearch.addEventListener('input', debouncedSearch);
    }
